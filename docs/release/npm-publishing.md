...
- The release workflow explicitly approves CI for the generated `changeset-release/main` head.
  Pull requests authored by the workflow token otherwise remain in `action_required`, and the
  protected branch requires the resulting `required` check before merge.
