...
describe("init command", () => {
	it("creates a minimal buildable project with Taplo schema associations", () => {
		...
		expect(result).toEqual({
			...
			files: [
				...
				"contexts/app/greeting.context.toml",
				".taplo.toml",
			],
			template: "minimal",
			locale: "en-US",
		...
		expect(readFileSync(join(directory, ".taplo.toml"), "utf8")).toContain(
			"s11tnext-authoring.schema.json",
		);
		expect(
			buildProject({ cwd: directory, releaseProfile: "development" }).catalogDigest,
	...
	it("creates a governed production starter with custom identifiers", () => {
		...
		expect(result.files).toEqual([
			"s11tnext.config.toml",
			"contexts/coding/agent/greeting.context.toml",
		]);
		expect(readFileSync(join(directory, "s11tnext.config.toml"), "utf8")).toContain(
			"require_owner = true",
		);
		expect(
			buildProject({ cwd: directory, releaseProfile: "production" }).checked,
		).toBe(false);
	});
	...
	it("previews without writing and refuses to overwrite existing files", () => {
		...
		expect(() => initProject({ cwd: directory })).toThrowError(
			expect.objectContaining<S11tnextDiagnosticError>({
				diagnostics: [
					expect.objectContaining({ code: "S11TNEXT_INIT_CONFLICT" }),
				],
			}),
		);
		expect(readFileSync(join(directory, "s11tnext.config.toml"), "utf8")).toBe(
			"preserve = true\n",
		);
	});
	...
	it("rejects invalid starter identifiers", () => {
		const directory = temporaryDirectory();
		expect(() => initProject({ cwd: directory, locale: "invalid locale" })).toThrow(
			S11tnextDiagnosticError,
		);
		expect(() => initProject({ cwd: directory, keyspace: "../escape" })).toThrow(
			S11tnextDiagnosticError,
		);
	...
	it.skipIf(process.platform === "win32")(
		"rejects a source path symlinked outside the project",
		() => {
			const directory = temporaryDirectory();
			...
			expect(() => initProject({ cwd: directory })).toThrowError(
				expect.objectContaining<S11tnextDiagnosticError>({
					diagnostics: [
						expect.objectContaining({ code: "S11TNEXT_INIT_UNSAFE_PATH" }),
					],
				}),
			);
		},
	...
	it.skipIf(process.platform === "win32")(
		"rejects a dangling source-directory symlink",
		() => {
			const directory = temporaryDirectory();
			const outside = temporaryDirectory();
